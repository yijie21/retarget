...
class Inference:
    ...
    def __call__(
        ...
        return self._pipeline.run(
            ...
            pointmap=pointmap,
            force_layout_post_optimization=force_layout_post_optimization,
            disable_shape_icp=disable_shape_icp,
        )
    
    def run_with_provided_object(self, image, masks, provided_object, with_mesh_postprocess=True, with_texture_baking=True, use_vertex_color=False, seed=None, pointmap=None, force_layout_post_optimization=False, disable_shape_icp=False):
        rgba_images = [self.merge_mask_to_rgba(image, mask) for mask in masks]
        
        processed_provided_object = None
        if provided_object is not None:
            processed_provided_object = provided_object.copy()
            if "mask" in provided_object:
                processed_provided_object["rgba_image"] = self.merge_mask_to_rgba(image, provided_object["mask"])
        
        return self._pipeline.run_with_provided_object(
            rgba_images,
            processed_provided_object,
            seed,
            with_mesh_postprocess=with_mesh_postprocess,
            with_texture_baking=with_texture_baking,
            with_layout_postprocess=True,
